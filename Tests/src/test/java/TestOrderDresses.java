...
import pages.MyAccountOrderDetails;
import pages.SignIn;
...
public class TestOrderDresses {
    ...
    @Test
    public void testOrderingOfDresses() throws InterruptedException {
        ...
        SignIn signIn = new SignIn(wd);

        System.out.println("Create account");
        signIn.createAccount((Random.getRandomUserEmail()), "Pera", "Peric", "peraperic1", "Adresa 1", "LA",
        ...
        MyAccountOrderDetails order = new MyAccountOrderDetails(wd);

        order.addToCart();
        order.proceedToCheckout();
        Assert.assertTrue(wd.getPageSource().contains("Your order on My Store is complete."));

        order.orderDetails();
//        System.out.println("Assert that the references are there.");
//        Assert.assertTrue(wd.getPageSource().contains("WWDTFQHUG"));
//        Assert.assertTrue(wd.getPageSource().contains("IMKTKAEOZ"));
